...
function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Agar data khali hai ya crash karne wala hai, to error dikhao
  ...
  // SAFE ACCESS HELPER
  const getSafeData = (path, fallback) => {
      ...
      <div className="canvas-container">
         <Canvas camera={{ position: [0, 0, 7.5], fov: 45 }}>
            ...
            {isMobile ? (
                ...
                <> <Stars radius={100} count={5000} factor={4} fade /> <Sparkles count={100} scale={6} size={2} color="#00e5ff" /> <Float speed={2} rotationIntensity={0.2} floatIntensity={0.2}><HolographicEarth /></Float> </>
            )}
        ...
        {showResult && plan && (
            <motion.div initial={{opacity: 0, scale: 0.9, y: 20}} animate={{opacity: 1, scale: 1, y: 0}} exit={{opacity: 0, scale: 0.9}} className="modal-overlay">
                <div className="modal-content">
                    ...
                    {/* SAFE ACCESS: ?. syntax prevents white screen */}
                    <p style={{textAlign: "center", color: "#888", fontSize: "1.2rem", marginBottom: "20px", fontWeight: "bold"}}>Total Budget: ₹{plan?.total_budget_estimated || "Calculating..."}</p>

                    <div style={{display: "flex", justifyContent: "center", gap: "15px", marginBottom: "20px"}}>
                        <button onClick={handleSpeak} style={{...actionBtn, background: isSpeaking ? "#ff4444" : "rgba(0, 229, 255, 0.1)"}}>{isSpeaking ? "🔇 Stop" : "🔊 Listen"}</button>
                        <button onClick={handleDownloadPDF} style={actionBtn}>📥 PDF</button>
                    </div>

                    <div style={gridContainer}>
                        <div style={card}><div style={label}>FLIGHT</div><div style={val}>{plan?.flight_selected?.airline || "Checking..."} (₹{plan?.flight_selected?.price || "N/A"})</div></div>
                    ...
                    <div style={{maxHeight: "200px", overflowY: "auto", paddingRight: "10px"}}>
                        {/* SAFE MAPPING: (list || []).map avoids crash */}
