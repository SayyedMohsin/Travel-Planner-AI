...
function App() {
  ...
  const handleDownloadPDF = () => {
      ...
      doc.setFontSize(12); doc.setTextColor(0); doc.text(`Total Budget: ${plan.total_budget_estimated} INR`, 20, 45);
      const flightName = plan.flight_selected?.airline || "Flight";
      doc.text(`Flight: ${flightName} (Cost: ${plan.flight_selected?.price || "N/A"})`, 20, 55);
      doc.text(`Hotel: ${plan.hotel_selected?.hotel_name || "Hotel"}`, 20, 65);
      ...
      plan.day_wise_plan.forEach((day) => {
          ...
          doc.setFont("helvetica", "bold"); doc.text(`Day ${day.day}`, 20, y); y += 7;
          doc.setFont("helvetica", "normal");
          const splitText = doc.splitTextToSize(day.activity || day.plan, 170); doc.text(splitText, 20, y); y += (splitText.length * 6) + 10;
      });
  ...
  const handleSpeak = () => {
      ...
      const text = `Namaste! Here is your plan for ${formData.destination}. Total budget is ${plan.total_budget_estimated} rupees. Enjoy your trip!`;
  ...
  return (
    ...
    <style>{`
        ...
        /* 1. New Background Gradient for Mobile (Deep Space Blue instead of Black) */
        ...
        .sidebar {
            ...
            /* No justify-space-between to prevent floating issues */
            z-index: 60; position: relative;
        }

        /* ABSOLUTE FOOTER FOR DESKTOP (Sticks to bottom) */
        .desktop-footer {
            position: absolute; bottom: 20px; left: 0; width: 100%;
            text-align: center; font-size: 0.75rem; color: #666; letter-spacing: 1px;
        ...
        .modal-content {
            width: 550px; background: #080c14; border: 1px solid #00e5ff;
            border-radius: 12px; padding: 40px; box-shadow: 0 0 60px rgba(0, 229, 255, 0.15);
        ...
        @media (max-width: 768px) {
            ...
            /* Mobile Footer also Absolute/Fixed at bottom of screen logic inside flex */
            .mobile-footer {
                margin-top: 20px; color: #888; font-size: 0.7rem; text-align: center;
            }
    ...
    <div className="app-container">
      ...
      <div className="canvas-container">
         <Canvas camera={{ position: [0, 0, 7.5], fov: 45 }}>
            {/* 2. New Sky Color for Mobile (Deep Blue Space) */}
            ...
            {isMobile ? (
                <>
                    {/* MOBILE: MORE STARS & NEBULA FEEL */}
                    <Stars radius={60} count={12000} factor={7} fade speed={2} />
                    <Sparkles count={500} scale={15} size={6} speed={0.5} color="#00e5ff" />
                    <Cloud opacity={0.2} speed={0.3} width={20} depth={2} segments={20} position={[0, 0, -10]} />
                </>
            ) : (
                <>
                    {/* DESKTOP */}
      ...
      <div className="sidebar">
        ...
        {/* WRAPPER (Card on Mobile, Normal Div on Desktop) */}
        ...
        {/* Desktop Footer (ABSOLUTE POSITION FIX) */}
        {!isMobile && (
        ...
        {showResult && plan && (
            <motion.div initial={{opacity: 0, scale: 0.9, y: 20}} animate={{opacity: 1, scale: 1, y: 0}} exit={{opacity: 0, scale: 0.9}} className="modal-overlay">
                <div className="modal-content">
                    <button onClick={() => {setShowResult(false); window.speechSynthesis.cancel(); setIsSpeaking(false);}} style={closeBtn}>×</button>
                    ...
                    <p style={{textAlign: "center", color: "#888", fontSize: "1.2rem", marginBottom: "20px", fontWeight: "bold"}}>Total Budget: ₹{plan.total_budget_estimated}</p>
                    <div style={{display: "flex", justifyContent: "center", gap: "15px", marginBottom: "20px"}}>
                        <button onClick={handleSpeak} style={{...actionBtn, background: isSpeaking ? "#ff4444" : "rgba(0, 229, 255, 0.1)"}}>
                            {isSpeaking ? "🔇 Stop" : "🔊 Listen"}
                        </button>
                        <button onClick={handleDownloadPDF} style={actionBtn}>📥 PDF</button>
                    </div>
                    <div style={gridContainer}>
                        <div style={card}><div style={label}>FLIGHT</div><div style={val}>{plan.flight_selected?.airline || "Flight"} (₹{plan.flight_selected?.price || "N/A"})</div></div>
                        <div style={card}><div style={label}>HOTEL</div><div style={val}>{plan.hotel_selected?.hotel_name || "Hotel"}</div></div>
                    </div>
